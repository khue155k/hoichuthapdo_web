...
export class TinhNguyenVienService {
  ...
  getAllTinhNguyenVienPaginated(pageSize: number, currentPage: number, sortBySoLanHien: boolean): Observable<TemplateResult<any>> {
    return this.http.get<TemplateResult<any>>(`${this.baseUrl}/getTNVsPaginated?pageSize=${pageSize}&currentPage=${currentPage}&sortBySoLanHien=${sortBySoLanHien}`);
  }
}
